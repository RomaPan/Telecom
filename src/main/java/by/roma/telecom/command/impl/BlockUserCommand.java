...
public class BlockUserCommand implements Command {
	...
	private static final String LOGGER_MESSAGE = "Service Exception occurred: failed to block user: ";
	private static final String BLOCK_USER = "BlockUserMessage";
	private static final String BLOCK_USER_SUCCESS = "User is blocked now";
	private static final String BLOCK_USER_FAIL = "User is not blocked. Please try again.";
	...
	public void execute(HttpServletRequest request, HttpServletResponse response) throws IOException, ServletException {
		...
		if (userID == null || userID.isEmpty()) {
			session.setAttribute(BLOCK_USER, BLOCK_USER_FAIL);
			response.sendRedirect(RedirectCommandName.GO_TO_USER_MANAGEMENT_PAGE);
			return;
		}
		try {
			user = userService.changeUserBlockStatus(userID, true);
			if (user == null) {
				session.setAttribute(BLOCK_USER, BLOCK_USER_FAIL);
				response.sendRedirect(RedirectCommandName.GO_TO_USER_MANAGEMENT_PAGE);
			} else {
				session.setAttribute("user", user);
				session.setAttribute(BLOCK_USER, BLOCK_USER_SUCCESS);
				response.sendRedirect(RedirectCommandName.GO_TO_USER_MANAGEMENT_PAGE);
			}
