...
<body>
		...
		<div id="localization-button">
			<form action="controller" method="post">
				<input type="hidden" name="command" value="localize" /> <input
					type="hidden" name="sitelocation" value="user-pagination" /> <input
	...
	<div class="spacer"> </div>
		<div class="system-message">
			<p>
				<span class="red"> 
					<c:out value="${UsersLisCleartMessage}" />
					<c:out value="${UsersListMessage}" />	   
				</span>
			</p>
		</div>
	<div class="spacer"></div>
	...
	<div id="pagin-nav">
		...
		<br>
		<p>
			<a href="?command=go-to-user-pagination&start=${pageStart - perPage}">${left_button}</a>
		</p>
		<p>${pageStart + 1}</p>
		<p>-</p>
		<p>${pageStart + perPage}</p>
		<p>
			<a href="?command=go-to-user-pagination&start=${pageStart + perPage}">${right_button}</a>
		</p>
	...
	<table id="admin-table">
		...
		<c:forEach var="u" items="${usersList}" begin="${pageStart}"
			...
			<tr>
				...
				<td>
					<c:if test="${not empty u.blocked}">
						<c:choose>
   				 			<c:when test="${u.blocked == true}">
       	 					<c:out value="${table_status_yes }" />
    					</c:when>    
    						<c:otherwise>
       							<c:out value="${table_status_no }" /> 
